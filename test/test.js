...
contract('Token', function(accounts) {
  ...
  it('devera passar se o contracto for depositado', async function() {
    let name = await contract.name.call();
    assert.strictEqual(name, 'Token');
  });
  it('deverá retornar o balanço iniciaal em wei de 1*10^27', async function() {
  ...
  it('deverá transferir o token', async function() {
  ...
  it('deverá retornar o total supply dos tokens', async function() {
    let totalSupply = await contract.totalSupply.call();
    totalSupply = totalSupply.toString();
    assert.strictEqual(totalSupply, '1e+27');
  });
  it('deverá aprovar o token de [transferFrom]', async function() {
    let approver = owner;
  ...
  it('deverá falhar para [transferFrom] more than allowed', async function() {
    let from = owner;
  ...
  it('deverá [transferFrom] tokens aprovados', async function() {
    let from = owner;
    let to = web3.eth.accounts[2];
    let spenderPrivateKey = privateKeys[2];
    let tokenWei = 5000000;
